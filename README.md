...
|---|---|---|---|
| Mean absolute error | persistence | 3.14 bp | 2.60 to 3.77 bp |
| CRPS | persistence | 2.56 bp | not intervalled |
| Interval coverage | nominal 90% | **81.0%** | 78.3% to 83.7% |
...
**The coverage line is the honest one, and it is a finding.** A nominal 90% interval covered 81.0% of 2080 realised outcomes. Intervalled the same way as the error above (stationary bootstrap, block length 5, 2000 replications, seed 507036806), the realised coverage lies between 78.3% and 83.7%, which excludes the nominal probability. What the gap is — a miscalibrated benchmark, or one a challenger will improve on — is open, and no verdict is asserted in the suite.
