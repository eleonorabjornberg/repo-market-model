...
- **Result.** The harness is verified end to end on real market data, the persistence
  ...
  weaker than persistence's and its tail exceedance probabilities are not yet scored, which
...
criterion is recorded met on the interval evidence**; its tail clause -- tail
exceedance probabilities, and climatology on a knowledge holdout -- is not scored.
