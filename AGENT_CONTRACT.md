...
- **Third-party packages live in `src/repo_model/ml.py` and nowhere else.**
  ...
  well: both conformance discoveries import every module of the package, so a
  module-level import there would fail the core suite without the extra. `ml.py`
  may omit `from __future__ import annotations`; the exceedance discovery reads
  either annotation form.
